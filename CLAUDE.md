...
The dashboard's four one-click actions (`clock_in`/`clock_out`/`start_pohyb`/`return_pohyb`, all in `timetracking/views.py`) accept an optional POST field `cas` ("HH:MM", always today's date — parsed by `_cas_z_pozadavku`/`_cas_nebo_chyba`) behind a "Změnit čas" UI toggle, letting an employee correct a late click without leaving the dashboard. Blank/absent `cas` is the default and behaves exactly like the old hardcoded `timezone.now()`. Two things a future edit here must preserve:
