...
- The same-day check in `_stejny_den_nebo_chyba` (used by `clock_out`/`return_pohyb`) only applies when `cas` was **explicitly entered** (`byl_zadan=True`), never to the default "now" path — otherwise a plain one-click Odchod/Návrat for a session or pohyb left open from a previous day would start failing at the view level. Note that since #57 the **dashboard no longer renders** Odchod/pohyb controls for a block started before today (it shows the forgotten-clock-out correction instead — see below), so this server path is now only reachable by a direct POST or a dashboard tab left open across midnight. Kept deliberately; blocking it server-side would be a separate decision.
...
### Forgotten clock-outs (correction flow)

`timetracking/opravy.py` is the single place for this: a record "k opravě" is any `WorkSession` or `Pohyb` with `konec IS NULL` whose `zacatek` is before today's local midnight (`zaznamy_k_oprave()`, regardless of whether `close_open_sessions` has flagged it yet). It's surfaced as a dated warning with "Opravit záznam"/"Opravit pohyb" on the dashboard (Odchod and pohyb controls hidden for that block — an overnight shift must be closed via the correction form too), a "Záznamy k opravě" section in Výkaz (own records) and "Záznamy k opravě v týmu" in the Odbor report. Correction views `opravit_session` / `opravit_pohyb` are allowed for the record's employee, `is_staff`, or a manager whose `spravovani_zamestnanci()` contains that employee (`muze_opravovat()` — the CRUD scope, never the read-only `viditelni_zamestnanci()`); the Odbor section uses the same scope. Saving a non-empty `konec` strips the `[AUTOMATICKY]` sentence (`ZNACKA_SESSION`/`ZNACKA_POHYB`, also imported by `close_open_sessions`) and keeps the employee's own note text. Both views honour a `next` parameter validated by `bezpecny_next()` (same-host only). A block with an open `Pohyb` can't be closed (`WorkSession.clean()`), so the dashboard links the pohyb correction first.
...
| Leave entitlements | `NarokDovolene` / `NarokIndispozicnihoVolna` are global (same for everyone); `aktivni_hodnota(datum)` returns the row with the newest `platne_od <= datum`. `TypStavu.vychozi_narok(datum)` supplies the default balance when no `ZustatekStavu` exists yet. `obnov_rocni_naroky`: dovolená = last year's leftover (min 0) + the yearly entitlement; indispoziční volno = reset to the current `NarokIndispozicnihoVolna` entitlement value — last year's leftover is **not** carried over and does not affect the new balance at all (unlike dovolená). Changing an entitlement does not recompute existing `ZustatekStavu` rows |
...
`close_open_sessions` flags `WorkSession` rows still open (`konec__isnull=True`) past a threshold (default 14h) by prepending an `[AUTOMATICKY]` note and setting `opraveno=False`. It does the same for open `Pohyb` rows past the threshold (an `[AUTOMATICKY]` note about the missing return time). It never closes anything — it only marks them for manual correction. It's idempotent: a row whose `poznamka` already starts with its `[AUTOMATICKY]` marker is not flagged again, only listed as still awaiting correction (before #55 every run prepended another copy). Times in its output go through `timezone.localtime()` — stored values are UTC.
